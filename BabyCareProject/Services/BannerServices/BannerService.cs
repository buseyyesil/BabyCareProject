...
namespace BabyCareProject.Services.BannerServices
{
    public class BannerService : IBannerService
    {
        ...
        {
            var values = await _bannerCollection.AsQueryable().OrderBy(x => x.Order).ToListAsync();
            return _mapper.Map<List<ResultBannerDto>>(values);
        }

        public async Task<List<ResultBannerDto>> GetActiveBannersAsync()
        {
            var values = await _bannerCollection.AsQueryable()
